...
if($action == 'delete')
{
	...
	if (!$confirm)
	{
		...
		$template->assign_vars(array(
			...
			'MESSAGE_TEXT' => (sizeof($extension) == 1) ? sprintf($user->lang['DL_CONFIRM_DELETE_EXTENSION'], $extension[0]) : sprintf($user->lang['DL_CONFIRM_DELETE_EXTENSIONS'], implode(', ', $extension)),

			'L_DELETE_FILE_TOO' => (sizeof($extension) == 1) ? $user->lang['DL_DELETE_EXTENSION_CONFIRM'] : $user->lang['DL_DELETE_EXTENSIONS_CONFIRM'],
	...
	else
	{
		...
		if (sizeof($sql_ext_in))
		{
			...
			$message = ((sizeof($extension) == 1) ? $user->lang['EXTENSION_REMOVED'] : $user->lang['EXTENSIONS_REMOVED']) . "<br /><br />" . sprintf($user->lang['CLICK_RETURN_EXTBLACKLISTADMIN'], '<a href="' . $basic_link . '">', '</a>') . adm_back_link($this->u_action);
