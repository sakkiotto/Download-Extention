...
if($action == 'edit' || $action == 'add')
{
	...
	if (isset($index[$cat_id]['allow_thumbs']) && $index[$cat_id]['allow_thumbs'] && $config['dl_thumb_fsize'])
	{
		...
		$thumbnail_explain	= sprintf($user->lang['DL_THUMB_DIM_SIZE'], $config['dl_thumb_xsize'], $config['dl_thumb_ysize'], \oxpus\dl_ext\includes\classes\ dl_format::dl_size($config['dl_thumb_fsize']));
...
else if($action == 'save')
{
	...
	else
	{
		...
		if ($config['dl_enable_blacklist'])
		{
			...
			{
				trigger_error($user->lang['DL_FORBIDDEN_EXTENSION'], E_USER_WARNING);
			}
		}
