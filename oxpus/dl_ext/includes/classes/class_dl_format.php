	...
	public static function rating_img($rating_points, $rate = false, $df_id = 0, $ext_path_images)
	{
		...
		for ($i = 0; $i < $config['dl_rate_points']; $i++)
		{
			...
			if ($rate)
			{
				...
				$rate_image .= ($j <= $rate_points ) ? '<a href="#" ' . $ajax . '>' . '<img src="' . $ext_path_images . 'dl_rate_yes.png" alt="' . $user->lang['IMG_DL_RATE_YES'] . '" />' . '</a>' : '<a href="#" ' . $ajax . '>' . '<img src="' . $ext_path_images . 'dl_rate_no.png" alt="' . $user->lang['IMG_DL_RATE_NO'] . '" />' . '</a>';

			}
			else
			{
				$rate_image .= ($j <= $rate_points ) ? '<img src="' . $ext_path_images . 'dl_rate_yes.png" alt="' . $user->lang['IMG_DL_RATE_YES'] . '" />' : '<img src="' . $ext_path_images . 'dl_rate_no.png" alt="' . $user->lang['IMG_DL_RATE_NO'] . '" />';
			}
		}
