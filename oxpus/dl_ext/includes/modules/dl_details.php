...
$inc_module = true;
page_header($this->user->lang['DOWNLOADS'] . ' - ' . $dl_files['description']);
...
if ($index[$cat_id]['comments'] && \oxpus\dl_ext\includes\classes\ dl_auth::cat_auth_comment_read($cat_id))
{
	...
	if ($this->config['dl_latest_comments'] && $real_comment_exists)
	{
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			{
				$edited_by = sprintf($this->user->lang['DL_COMMENT_EDITED'], $this->user->format_date($comment_edit_time));
			}
			else
			{
				$edited_by = '';
...
$last_time_string		= ($dl_files['extern']) ? $this->user->lang['DL_LAST_TIME_EXTERN'] : $this->user->lang['DL_LAST_TIME'];
$last_time				= ($dl_files['last_time']) ? sprintf($last_time_string, $this->user->format_date($dl_files['last_time'])) : $this->user->lang['DL_NO_LAST_TIME'];
...
if ($dl_files['hacklist'] && $this->config['dl_use_hacklist'])
{
	$this->template->assign_block_vars('hacklist', array(
		...
		'HACK_AUTHOR_WEBSITE'	=> ( $hack_author_website != '' ) ? ' [ <a href="'.$hack_author_website.'">'.$this->user->lang['WEBSITE'].'</a> ]' : '',
		'HACK_DL_URL'	=> ( $hack_dl_url != '' ) ? '<a href="' . $hack_dl_url . '">'.$this->user->lang['DL_DOWNLOAD'].'</a>' : 'n/a')
	);
}
...
$this->template->assign_block_vars('downloads', array(
	...
	'FILE_NAME'				=> ($dl_files['extern']) ? $this->user->lang['DL_EXTERN'] : $file_name,
	'LAST_TIME'				=> $last_time,
	'ADD_USER'				=> ($add_user != '') ? sprintf($this->user->lang['DL_ADD_USER'], $add_time, $add_user) : '',
	'CHANGE_USER'			=> ($change_user != '') ? sprintf($this->user->lang['DL_CHANGE_USER'], $change_time, $change_user) : '')
);
...
if ($this->user->data['is_registered'] && !$this->config['dl_disable_email'])
{
	...
	{
		$l_favorite = $this->user->lang['DL_FAVORITE_ADD'];
