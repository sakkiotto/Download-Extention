		...
		*/
		if ($action == 'save' && $submit)
		{
			...
			if ($file_option == 3)
			{
				...
				{
					trigger_error($this->user->lang['DL_VER_DEL_ERROR'], E_USER_ERROR);
				}
				...
				if (!$confirm)
				{
					...
					*/
					page_header($this->user->lang['DL_DELETE_FILE_CONFIRM']);
					...
					$this->template->assign_vars(array(
						...
						'MESSAGE_TEXT' => sprintf($this->user->lang['DL_CONFIRM_DELETE_VERSIONS']),
			...
			else
			{
				...
				if (!$description)
				{
					trigger_error($this->user->lang['NO_SUBJECT'], E_USER_WARNING);
				}		
				...
				if (!$file_extern)
				{
					...
					if ($this->config['dl_enable_blacklist'])
					{
						...
						{
							trigger_error($this->user->lang['DL_FORBIDDEN_EXTENSION'], E_USER_ERROR);
						}
					}
					...
					if ($file_name)
					{
						if (!$this->config['dl_traffic_off'])
						{
							...
							{
								trigger_error($this->user->lang['DL_NO_UPLOAD_TRAFFIC'], E_USER_ERROR);
							}
						}
			...
			if (!empty($dl_id))
			{
				if (!$confirm)
				{
					...
					else
					{
						...
						$delete_confirm_text	= $this->user->lang['DL_CONFIRM_DELETE_MULTIPLE_FILES'];
					}
		...
		if ($action == 'edit')
		{
			...
			$s_check_free .= '<option value="0">' . $this->user->lang['NO'] . '</option>';
			$s_check_free .= '<option value="1">' . $this->user->lang['YES'] . '</option>';
			$s_check_free .= '<option value="2">' . $this->user->lang['DL_IS_FREE_REG'] . '</option>';
			$s_check_free .= '</select>';
		
			$s_traffic_range = '<select name="dl_t_quote">';
			$s_traffic_range .= '<option value="byte">' . $this->user->lang['DL_BYTES'] . '</option>';
			$s_traffic_range .= '<option value="kb">' . $this->user->lang['DL_KB'] . '</option>';
			$s_traffic_range .= '<option value="mb">' . $this->user->lang['DL_MB'] . '</option>';
			$s_traffic_range .= '<option value="gb">' . $this->user->lang['DL_GB'] . '</option>';
			$s_traffic_range .= '</select>';
		
			$s_file_ext_size_range = '<select name="dl_e_quote">';
			$s_file_ext_size_range .= '<option value="byte">' . $this->user->lang['DL_BYTES'] . '</option>';
			$s_file_ext_size_range .= '<option value="kb">' . $this->user->lang['DL_KB'] . '</option>';
			$s_file_ext_size_range .= '<option value="mb">' . $this->user->lang['DL_MB'] . '</option>';
			$s_file_ext_size_range .= '<option value="gb">' . $this->user->lang['DL_GB'] . '</option>';
			$s_file_ext_size_range .= '</select>';
		
			$s_hacklist = '<select name="hacklist">';
			$s_hacklist .= '<option value="0">' . $this->user->lang['NO'] . '</option>';
			$s_hacklist .= '<option value="1">' . $this->user->lang['YES'] . '</option>';
			$s_hacklist .= '<option value="2">' . $this->user->lang['DL_MOD_LIST'] . '</option>';
			$s_hacklist .= '</select>';
			...
			page_header($this->user->lang['DL_MODCP_EDIT']);
			...
			{
				$blacklist_explain = '<br />' . sprintf($this->user->lang['DL_FORBIDDEN_EXT_EXPLAIN'], implode(', ', $ext_blacklist));
			}
			else
			{
				$blacklist_explain = '';
			...
			$dl_files_page_title = $this->user->lang['DL_FILES_TITLE'];
			...
			$this->template->assign_vars(array(
				...
				'MAX_UPLOAD_SIZE' => sprintf($this->user->lang['DL_UPLOAD_MAX_FILESIZE'], \oxpus\dl_ext\includes\classes\ dl_physical::dl_max_upload_size()),
		...
		if ($action == 'approve')
		{
			...
			page_header($this->user->lang['DL_MODCP_APPROVE']);
		...
		if ($action == 'capprove')
		{
			...
			page_header($this->user->lang['DL_MODCP_CAPPROVE']);
		...
		if ($action == 'manage' && $cat_id)
		{
			...
			page_header($this->user->lang['DL_MODCP_MANAGE']);
			...
			$this->template->assign_vars(array(
				'DL_UP'			=> ($sort && ($this->auth->acl_get('a_') && $this->user->data['is_registered'])) ? $this->user->lang['DL_UP'] : '',
				'DL_DOWN'		=> ($sort && ($this->auth->acl_get('a_') && $this->user->data['is_registered'])) ? $this->user->lang['DL_DOWN'] : '',
				'DL_ABC'		=> ($this->auth->acl_get('a_') && $this->user->data['is_registered']) ? $this->user->lang['SORT_BY'] . ' ASC' : '',
