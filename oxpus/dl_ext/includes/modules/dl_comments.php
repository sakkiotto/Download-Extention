...
{
	$inc_module = true;
	$page_title = $this->user->lang['DL_COMMENTS'];
	...
	if ($action == 'save' && !$deny_post)
	{
		...
		if ($dl_id)
		{
			...
			$comment_message = $this->user->lang['DL_COMMENT_UPDATED'];
		}
		else
		{
			...
			$comment_message = $this->user->lang['DL_COMMENT_ADDED'];
		}
		...
		if (!$approve)
		{
			...
			$return_text		= $this->user->lang['CLICK_RETURN_COMMENTS'];
		}
		else
		{
			...
			$approve_message	= '<br />' . $this->user->lang['DL_MUST_BE_APPROVE_COMMENT'];
			$return_parameters	= array('view' => 'detail', 'df_id' => $df_id);
			$return_text		= $this->user->lang['CLICK_RETURN_DOWNLOAD_DETAILS'];
		}
	...
	if ($action == 'delete' && $allow_manage)
	{
		...
		if (!$confirm)
		{
			...
			page_header($this->user->lang['DOWNLOADS'] . ' :: ' . $this->user->lang['DELETE_MESSAGE']);
	
			add_form_key('dl_com_del');
	
			$this->template->assign_vars(array(
				'MESSAGE_TITLE' => $this->user->lang['DELETE_MESSAGE'],
				'MESSAGE_TEXT' => $this->user->lang['DELETE_MESSAGE_CONFIRM'],
	...
	if (($action == 'edit' && $allow_manage) || ($action == 'post' && !$deny_post))
	{
		...
		page_header($this->user->lang['DOWNLOADS'] . ' :: ' . $this->user->lang['DL_COMMENT']);
	}
	...
	if ($action == 'view' || !$action)
	{
		...
		*/
		...
		if ($total_comments)
		{
			...
			page_header($this->user->lang['DOWNLOADS'] . ' :: ' . $this->user->lang['DL_COMMENTS']);
			...
			$this->template->assign_vars(array(
				'L_POST_COMMENT'	=> ($deny_post) ? '' : $this->user->lang['DL_COMMENT_WRITE'],
