...
page_header($this->user->lang['DL_HACKS_LIST']);
	...
	for ($i = 0; $i < sizeof($dl_files); $i++)
	{
		...
		if ($hacklist[$cat_id])
		{
			...
			$this->template->assign_block_vars('listrow', array(
				...
				'HACK_AUTHOR_WEBSITE'	=> ($hack_author_website != '') ? '<a href="' . $hack_author_website . '">' . $this->user->lang['DL_HACK_AUTOR_WEBSITE'] . '</a>' : '',
				'HACK_DL_URL'			=> ($hack_dl_url != '') ? '<a href="' . $hack_dl_url . '">' . $this->user->lang['DL_DOWNLOAD'] . '</a>' : '')
			);
		}
	}
}
