...
if ($preview && $this->user->data['is_registered'])
{
	...
	if (!$report_title)
	{
		trigger_error($this->user->lang['DL_BUG_REPORT_NO_TITLE'], E_USER_WARNING);
	}

	if (!$report_text)
	{
		trigger_error($this->user->lang['DL_BUG_REPORT_NO_TEXT'], E_USER_WARNING);
	}
...
if ($action == 'save' && $this->user->data['is_registered'])
{
	...
	$message = $this->user->lang['DL_BUG_REPORT_ADDED'] . '<br /><br />' . sprintf($this->user->lang['CLICK_RETURN_BUG_TRACKER'], '<a href="' . $this->helper->route('dl_ext_controller', $link_array) . '">', '</a>');
...
if ($action == 'assign' && $df_id && $fav_id && $allow_bug_mod)
{
	...
	if (!$new_user_id)
	{
		trigger_error($this->user->lang['DL_NO_PERMISSIONS'], E_USER_WARNING);
	}
...
if ($action == 'detail' && $fav_id)
{
	...
	$this->template->assign_vars(array(
		...
		'REPORT_STATUS'		=> $this->user->lang['DL_REPORT_STATUS'][$report_status],
	...
	if ($total_history)
	{
		...
		while ($row = $this->db->sql_fetchrow($result))
		{
			...
			{
				$output_value = $this->user->lang['DL_BUG_REPORT_ASSIGN'];
				$output_data = explode(':', $report_his_value);

				$output_text = $this->user->lang['DL_BUG_REPORT_ASSIGNED'] . ' -> ' . $output_data[1];
			}
			else if ($report_his_type == 'status')
			{
				$output_value = $this->user->lang['DL_BUG_REPORT_STATUS'];
				$output_data = explode(':', $report_his_value);

				$output_status = intval($output_data[0]);
				$output_text = $this->user->lang['DL_REPORT_STATUS'][$output_status] . ' -> ' . $output_data[1];
	...
	if ($allow_bug_mod)
	{
		...
		switch ($report_status)
		{
			...
			case 2:
			case 3:
				$s_select_status .= '<option value="2">'.$this->user->lang['DL_REPORT_STATUS'][2].'</option>';
				$s_select_status .= '<option value="3">'.$this->user->lang['DL_REPORT_STATUS'][3].'</option>';
				$s_select_status .= '<option value="4">'.$this->user->lang['DL_REPORT_STATUS'][4].'</option>';
				$s_select_status .= '<option value="5">'.$this->user->lang['DL_REPORT_STATUS'][5].'</option>';
				break;
			case 4:
			case 5:
		}
...
if ($action == 'delete' && $df_id && $fav_id && $allow_bug_mod)
{
	if (!$confirm && !$cancel)
	{
		...
		$this->template->assign_vars(array(
			...
			'MESSAGE_TEXT' => $this->user->lang['DL_CONFIRM_DELETE_BUG_REPORT'],
...
if (!$action)
{
	...
	$s_select_filter .= '<option value="0">'.$this->user->lang['DL_NO_FILTER'].'</option>';
	$s_select_filter .= '<option value="-1">'.$this->user->lang['DL_FILTER_OPEN'].'</option>';
	$s_select_filter .= '<option value="1">'.$this->user->lang['DL_REPORT_STATUS'][0].$bug_status_count[0].'</option>';
	$s_select_filter .= '<option value="2">'.$this->user->lang['DL_REPORT_STATUS'][1].$bug_status_count[1].'</option>';
	$s_select_filter .= '<option value="3">'.$this->user->lang['DL_REPORT_STATUS'][2].$bug_status_count[2].'</option>';
	$s_select_filter .= '<option value="4">'.$this->user->lang['DL_REPORT_STATUS'][3].$bug_status_count[3].'</option>';
	$s_select_filter .= '<option value="5">'.$this->user->lang['DL_REPORT_STATUS'][4].$bug_status_count[4].'</option>';
	$s_select_filter .= '<option value="6">'.$this->user->lang['DL_REPORT_STATUS'][5].$bug_status_count[5].'</option>';
