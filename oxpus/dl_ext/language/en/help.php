...
$lang = array_merge($lang, array(
	...
	'HELP_DL_ENABLE_RATE'			=> 'With this option you can enable/disable the rating system.<br />If there are still exists rating points they will not be deleted and the Extension shown them immediatly if you enable the rating system again.',
	...
	'HELP_DL_FILES_URL'			=> 'The filename of this download.<br />Enter this name without a leading file path or slash.<br />The file must exist bevore saving this download otherwise you will get an error message.<br />Note forbidden file Extensions: Each file which have a forbidden Extension will be blocked.',
	...
	'HELP_DL_MOD_TEST'			=> 'Declare on which phpBB this Extension was tested successfull.<br />Just enter the release from the test board.<br />The script will display it as phpBB X, so you must only enter X here.<br />This text will only be shown in the download details.',
	...
	'HELP_DL_MOD_WARNING'		=> 'Important advices about this Extension which must be regarded on installation, using or interacton with other Extensions.<br />This text will be shown in the download details formatted with an other color (per default the font color is red).<br />Line feeds will be formatted.<br />BBCodes are not aviable here.',
	...
	'HELP_DL_ON_ADMINS'					=> 'Allows board administrators to enter the downloads and work within while the download Extension is deactivated.<br />Otherwise the adminstrators will be locked out, too.',
	...
	'HELP_DL_UPLOAD_FILE'			=> 'The upload file from your computer.<br />Be sure, the filesize will be smaller as shown and the file Extension will not included the list you can see under this field.',
	...
	'HELP_DOWNLOAD_PATH'			=> 'The relative path under your phpBB root folder.<br />On first installation of this package the storage will be set to "forumroot/ext/oxpus/dl_ext/files/downloads/".This can be changed in the ACP within the extiosion settings to "forumroot/store/oxpus/dl_ext/downloads/".<br />Regard if you will still manage a huge number of downloads the automatic storage movement will take a while or can produce errors. In this case activate the next option to switch over to the alternative storage place and move the files manually into the destination folder (e. g. within a FTP client).<br />Under this basic folder you must create one or more subfolders which will contains all physical files.<br />It is recommended to create a subfolder for each logical group of categories.<br />This subfolder must be entered as path in the category with the same syntax as the main folder, but without using the main folder (for more read the help in the category management).<br />You can create more subfolders by using a ftp client or by using the Toolbox (see the dropdown on the top right of this page).',
